...
import { z } from 'zod'
import { ReleaseNotesPreview } from './globals/ReleaseNotesPreview'
...
export default buildConfig({
  admin: {
    components: {
      ...
      afterNavLinks: ['@/components/afterNavLinks/LinkToContentHealth#LinkToContentHealth'],
      // Add custom admin views
      views: {
        contentHealth: {
          Component: '/components/views/ContentHealth/index.js#ContentHealthView',
          path: '/content-health',
          exact: true,
        },
      },
  ...
  globals: [Header, Footer, ReleaseNotesPreview],
        ...
        tools: [
          {
            ...
            description: 'Analyze and summarize the latest posts with 3 bullet points each',
            ...
            parameters: z.object({
              limit: z
                ...
                .max(50)
                .default(5)
                .describe('How many recent posts to include (max 50).'),
            }).shape,
            ...
            handler: async (args: Record<string, unknown>) => {
              ...
              const text = `To draft release notes for the latest ${limit} posts, please:

1. Use the "findPosts" tool with limit=${limit} and sort="-createdAt" to get the most recent posts
2. For each post, analyze its content thoroughly
3. Format each post as follows:
   - **[Post Title]** — [Date in YYYY-MM-DD format]
     - [First key takeaway or main point from the post]
     - [Second key takeaway or important detail]
     - [Third key takeaway or notable insight]

4. Return the complete formatted list with all posts analyzed

Your analysis should focus on extracting the most important and actionable information from each post's content.`
          ...
          },
          {
            ...
            description: 'Analyze a post and suggest SEO-friendly meta title and description',
            // IMPORTANT: pass a ZodRawShape by using `.shape`
            parameters: z.object({
              slug: z
                .string()
                .min(1)
                .describe('The slug of the post to analyze for SEO suggestions'),
            }).shape,
            ...
            handler: async (args: Record<string, unknown>) => {
              ...
              // Return instructions for the MCP client to use the built-in findPosts tool
              const text = `To suggest SEO metadata for the post with slug "${slug}", please:

1. Use the "findPosts" tool with where[slug][equals]="${slug}" to find the specific post
2. Analyze the post's content, title, and main topics thoroughly
3. Generate SEO-optimized suggestions following these best practices:

   **SEO Guidelines:**
   - Meta Title: 50-60 characters (optimal display in search results)
   - Meta Description: 150-160 characters (optimal display in search results)
...
4. Format your response as follows:
   ...
   **Suggested Meta Title** (50-60 chars):
   \`[Your suggested meta title]\`
   Character count: [X]

   **Suggested Meta Description** (150-160 chars):
   \`[Your suggested meta description]\`
   Character count: [X]

   **Alternative Meta Title** (50-60 chars):
   \`[Alternative suggestion]\`
   Character count: [X]

   **Alternative Meta Description** (150-160 chars):
   \`[Alternative suggestion]\`
   Character count: [X]

   **Key SEO Insights:**
   - Primary keywords identified: [list keywords]
   - Target audience: [who this content is for]
   - Search intent: [informational/transactional/navigational]
   - Recommended improvements: [any content suggestions]

5. If the post is not found, return a message indicating that no post with that slug exists

Focus on creating metadata that balances SEO optimization with compelling copy that drives clicks.`
